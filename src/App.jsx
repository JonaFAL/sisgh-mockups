...
const emptyRow = () => ({
  id: Date.now()+Math.random(), titulo:"", item:"", subitem:"", tipoDato:"T",
  ...
  opciones:["","","","","",""], plantilla:"", soloUna:false, clave:false,
  unidad:"", rangoMinN:"", rangoMaxN:"",
});
const defaultForm = () => ({
  id: Date.now()+Math.random(), estudio:"", titulo:"", especialidad:"INFECTOLOGIA",
  categoria:"Evolución Enfermería", copias:1, rows:[emptyRow()],
  datoComplementario:false, informeComplementario:false, esMuestra:false,
  requiereAutorizacion:false, auditable:false, activo:true, bistand:true,
});
const defaultProject = () => ({
  paciente: { nombre:"", hc:"", edad:"", sexo:"Fem", fechaNacimiento:"" },
  internacion: { episodio:"", ingreso:"", ubicacion:"", egreso:"", cantDias:"" },
  indicadores: [], problemas: [],
  formularios: [defaultForm()],
  sheetUrl: "",
  destinatario: "",
});

const S = {
  bg:"#c0c0c0", bgDark:"#a0a0a0", borderLight:"#dfdfdf", borderDark:"#808080",
...
// === VFP COMPONENTS ===
const VBtn=({children,onClick,style,active,disabled,small})=>(
  <button onClick={onClick} disabled={disabled} style={{fontFamily:S.font,fontSize:small?"10px":"11px",padding:small?"2px 8px":"3px 12px",border:active?`2px inset ${S.borderDark}`:`2px outset ${S.borderLight}`,background:active?S.bgDark:S.bg,cursor:disabled?"default":"pointer",color:disabled?"#808080":"#000",whiteSpace:"nowrap",...style}}>{children}</button>
);
const VInp=({value,onChange,style,placeholder})=>(
  <input value={value} onChange={onChange} placeholder={placeholder} style={{fontFamily:S.font,fontSize:"11px",border:`2px inset ${S.borderDark}`,padding:"2px 4px",background:"#fff",...style}}/>
);
const VSel=({value,onChange,options,style})=>(
  <select value={value} onChange={onChange} style={{fontFamily:S.font,fontSize:"11px",border:`2px inset ${S.borderDark}`,padding:"1px 2px",background:"#fff",...style}}>
    {options.map(o=>(<option key={typeof o==="string"?o:o.value} value={typeof o==="string"?o:o.value}>{typeof o==="string"?o:o.label}</option>))}
  </select>
);
const VChk=({checked,onChange,label})=>(
  ...
  if(tipo.group==="buleano") return(
    <div style={box}>
      ...
      <div style={{display:"flex",gap:"16px",flexWrap:"wrap"}}>
        <div><div style={{fontSize:"9px",color:"#444",marginBottom:"2px"}}>Positivo</div><input value={row.positivo} onChange={e=>upd("positivo",e.target.value)} style={{...mi,width:"100px"}}/></div>
        <div><div style={{fontSize:"9px",color:"#444",marginBottom:"2px"}}>Negativo</div><input value={row.negativo} onChange={e=>upd("negativo",e.target.value)} style={{...mi,width:"100px"}}/></div>
        <div><div style={{fontSize:"9px",color:"#444",marginBottom:"2px"}}>Rango Mín</div><input value={row.rangoMin} onChange={e=>upd("rangoMin",e.target.value)} style={{...mi,width:"70px"}}/></div>
        <div><div style={{fontSize:"9px",color:"#444",marginBottom:"2px"}}>Rango Máx</div><input value={row.rangoMax} onChange={e=>upd("rangoMax",e.target.value)} style={{...mi,width:"70px"}}/></div>
      </div>
    </div>
  );
  if(tipo.group==="combo") return(
    <div style={box}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"6px"}}>
        {row.opciones.map((op,i)=>(<div key={i}><div style={{fontSize:"9px",color:"#444",marginBottom:"2px"}}>Opción {i+1}</div><input value={op} onChange={e=>updOpc(i,e.target.value)} style={{...mi,width:"100%"}} placeholder={`Opción ${i+1}...`}/></div>))}
      </div>
    </div>
  );
  if(tipo.group==="texto") return(
    <div style={box}>
      ...
      <div style={{display:"flex",gap:"16px",alignItems:"start",flexWrap:"wrap"}}>
        <div style={{flex:1,minWidth:"200px"}}><div style={{fontSize:"9px",color:"#444",marginBottom:"2px"}}>Plantilla</div><textarea value={row.plantilla} onChange={e=>upd("plantilla",e.target.value)} rows={2} style={{...mi,width:"100%",resize:"vertical"}} placeholder="Texto predeterminado..."/></div>
        <div style={{display:"flex",flexDirection:"column",gap:"4px",paddingTop:"14px"}}><VChk checked={row.soloUna} onChange={e=>upd("soloUna",e.target.checked)} label="Solo Una"/><VChk checked={row.clave} onChange={e=>upd("clave",e.target.checked)} label="Clave"/></div>
      </div>
    </div>
  );
  if(tipo.group==="numerico") return(
    <div style={box}>
      ...
      <div style={{display:"flex",gap:"16px",flexWrap:"wrap",alignItems:"end"}}>
        <div><div style={{fontSize:"9px",color:"#444",marginBottom:"2px"}}>Unidad</div><input value={row.unidad} onChange={e=>upd("unidad",e.target.value)} style={{...mi,width:"80px"}} placeholder="mg/dl"/></div>
        <div><div style={{fontSize:"9px",color:"#444",marginBottom:"2px"}}>Mín</div><input value={row.rangoMinN} onChange={e=>upd("rangoMinN",e.target.value)} style={{...mi,width:"70px"}}/></div>
        <div><div style={{fontSize:"9px",color:"#444",marginBottom:"2px"}}>Máx</div><input value={row.rangoMaxN} onChange={e=>upd("rangoMaxN",e.target.value)} style={{...mi,width:"70px"}}/></div>
      ...
      <div onClick={e=>e.stopPropagation()} style={{background:S.bg,border:`2px outset ${S.borderLight}`,width:"680px",maxWidth:"95vw",maxHeight:"90vh",display:"flex",flexDirection:"column",fontFamily:S.font,fontSize:"11px"}}>
        ...
        <div style={{padding:"10px",overflow:"auto",flex:1}}>
          ...
          <div style={{display:"flex",gap:"12px",marginBottom:"6px"}}>
            {[["tab","Tab (Excel)"],["semicolon",";"],["comma",","]].map(([v,l])=>(<label key={v} style={{display:"flex",alignItems:"center",gap:"3px",fontSize:"10px",cursor:"pointer"}}><input type="radio" checked={sep===v} onChange={()=>setSep(v)}/>{l}</label>))}
          </div>
          ...
          {preview.length>0&&(<div style={{marginTop:"8px"}}><b style={{color:S.blue}}>{preview.length} filas detectadas</b>
            <div style={{border:`2px inset ${S.borderDark}`,background:"#fff",marginTop:"4px",maxHeight:"150px",overflowY:"auto",fontSize:"10px"}}>
              {preview.map((r,i)=>{const td=TIPOS_DATO.find(t=>t.code===r.tipoDato);return(
                <div key={i} style={{display:"flex",gap:"8px",padding:"2px 4px",borderBottom:"1px solid #eee",background:i%2?"#f8f8f8":"#fff"}}>
                  ...
                  <span style={{width:"80px"}}><span style={{background:"#e0e0ff",padding:"0 3px",fontSize:"9px"}}>{r.tipoDato}</span> {td?.label?.split("(")[0]||""}</span>
                </div>
              );})}
            </div>
          </div>)}
        ...
        <div style={{display:"flex",gap:"6px",justifyContent:"flex-end",padding:"8px",borderTop:`1px solid ${S.borderDark}`}}>
          ...
          <VBtn disabled={!preview.length} onClick={()=>{onImport(preview);onClose();}} small style={{background:"#d0d0ff",fontWeight:"bold"}}>✅ Importar {preview.length} filas</VBtn>
        </div>
      </div>
    </div>
  );
  ...
  return(
    <div>
      <div style={{display:"grid",gridTemplateColumns:"auto 1fr auto 1fr",gap:"4px 8px",alignItems:"center",marginBottom:"6px"}}>
        <b>Estudio</b><VInp value={form.estudio} onChange={e=>upd("estudio",e.target.value)} placeholder="AISLAMIENTO - RESPIRATORIO AÉREO" style={{width:"100%"}}/>
        <b>Título</b><VInp value={form.titulo} onChange={e=>upd("titulo",e.target.value)} placeholder="COMITE DE CONTROL DE INFECCIONES" style={{width:"100%"}}/>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"auto 1fr auto 1fr",gap:"4px 8px",alignItems:"center",marginBottom:"6px"}}>
        <b>Especialidad</b><VSel value={form.especialidad} onChange={e=>upd("especialidad",e.target.value)} options={ESPECIALIDADES}/>
        <b>Categoría</b><VSel value={form.categoria} onChange={e=>upd("categoria",e.target.value)} options={CATEGORIAS}/>
      </div>
      <div style={{display:"flex",gap:"4px",marginBottom:"4px",flexWrap:"wrap",alignItems:"center"}}>
        ...
        <div style={{flex:1}}/>
        <span style={{fontSize:"10px",color:"#666"}}>{form.rows.length} filas</span>
      </div>
            ...
            return(<div key={row.id}>
              ...
              {isExp&&(<div style={{background:"#f0f0f8",borderBottom:`2px solid ${S.blue}`,padding:"8px 10px"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"4px"}}>
                  ...
                  <div style={{display:"flex",gap:"3px"}}><VBtn small onClick={()=>dupRow(idx)}>📋 Dup</VBtn><VBtn small onClick={()=>removeRow(idx)} style={{color:"#800000"}}>🗑️</VBtn></div>
                </div>
      ...
      {showImport&&<ImportModal onClose={()=>setShowImport(false)} onImport={imported=>{
        const nr=imported.map(r=>({...emptyRow(),...r,id:Date.now()+Math.random()}));
        setForm({...form,rows:[...form.rows,...nr]});
      }}/>}
    </div>
  );
}

// === MOCKUP REGISTRO VIEW ===
function RegistroView({form,sheetUrl,onBack,paciente}){
  ...
  const sendToSheet=async()=>{
    if(!sheetUrl){alert("No hay URL de Google Sheet configurada. Contacte al administrador.");return;}
    setSending(true);
    try{
      const payload={
        revisor:sender||"(anónimo)",
        estudio:form.estudio||"(sin estudio)",
        notaGeneral:nota,
    ...
    }catch(err){alert("❌ Error al enviar. Intentá de nuevo o contactá al administrador.");}
    setSending(false);
  };
  ...
  const Cell=({field,children,style})=>(
    <div onClick={e=>openPopup(field,e)} style={{...style,position:"relative",cursor:commentMode?"pointer":"default",outline:comments[field]?"2px solid #ff4400":"none"}}
      ...
      {children}{comments[field]&&<span style={{position:"absolute",top:0,right:2,fontSize:"10px"}}>💬</span>}
    </div>
  );
    ...
    <div>
        ...
        <div style={{padding:"6px 8px",fontSize:"11px"}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr auto auto",border:`1px solid ${S.borderDark}`,marginBottom:"4px"}}>
            <div style={{padding:"4px 8px",borderRight:`1px solid ${S.borderDark}`}}>
              ...
              <div style={{fontSize:"18px",fontWeight:"bold"}}>{paciente.nombre||"[Nombre]"}</div>
              <div style={{display:"flex",gap:"8px",marginTop:"2px",fontSize:"10px"}}>
                <span><span style={{color:S.teal}}>HC:</span> <b>{paciente.hc||"[HC]"}</b></span>
                <span><span style={{color:S.teal}}>Edad:</span> <b>{paciente.edad||"[Edad]"}</b></span>
                <span><span style={{color:S.teal}}>Sexo:</span> <b>{paciente.sexo||"[Sexo]"}</b></span>
              </div>
            </div>
          ...
          <div style={{border:`2px inset ${S.borderDark}`,background:"#fff"}}>
            <div style={{display:"grid",gridTemplateColumns:"140px 170px 1fr 24px 24px 200px",background:S.gridHeader,color:"#fff",fontWeight:"bold",fontSize:"10px"}}>
              {["Título","Item","SubItem","P","R","Resultado"].map(h=>(<div key={h} style={{padding:"3px 4px",borderRight:"1px solid #4040c0",textAlign:"center"}}>{h}</div>))}
            </div>
            <div style={{minHeight:"120px"}}>
              {form.rows.map((row,idx)=>{
                const fp=`Fila ${idx+1}: ${row.titulo||"(sin título)"}`;
                return(<div key={row.id} style={{display:"grid",gridTemplateColumns:"140px 170px 1fr 24px 24px 200px",borderBottom:"1px solid #c0c0c0",minHeight:"28px",fontSize:"10px"}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",padding:"3px 4px",border:`2px inset ${S.borderDark}`,marginTop:"4px",fontSize:"9px"}}>
            <span>{[["#0a0","Obligatorio"],["#cc0","Incompleto"],["#f00","Fuera Rango"],["#888","Por Defecto"],["#00f","Fórmula"]].map(([c,l])=><span key={l} style={{marginRight:"6px"}}><span style={{width:"7px",height:"7px",borderRadius:"50%",background:c,display:"inline-block",marginRight:"2px"}}/>{l}</span>)}</span>
      ...
      {popup&&(<div style={{position:"fixed",left:popup.x,top:popup.y,zIndex:1000,background:"#ffffee",border:`2px outset ${S.borderLight}`,padding:"8px",boxShadow:"3px 3px 6px rgba(0,0,0,0.3)",width:"320px",fontFamily:S.font,fontSize:"11px"}}>
        ...
        <select value={popupType} onChange={e=>setPopupType(e.target.value)} style={{width:"100%",fontSize:"11px",border:`2px inset ${S.borderDark}`,marginBottom:"6px",padding:"2px",fontFamily:S.font}}>
          <option value="corregir">🔴 Corregir</option><option value="sugerir">🔵 Sugerencia</option><option value="consulta">🟢 Consulta</option><option value="agregar">🟠 Falta algo</option>
        </select>
      ...
      {showSend&&(<div style={{border:`2px outset ${S.borderLight}`,background:"#d4d0c8",padding:"10px",marginTop:"6px",fontFamily:S.font,fontSize:"11px"}}>
        ...
        <div style={{background:"#fff",border:`2px inset ${S.borderDark}`,padding:"6px",maxHeight:"150px",overflowY:"auto",fontSize:"10px"}}>
          {Object.entries(comments).map(([f,d])=>(<div key={f} style={{padding:"3px 0",borderBottom:"1px dotted #c0c0c0"}}><b style={{color:S.blue}}>{f}</b> <span style={{display:"inline-block",padding:"0 4px",fontSize:"9px",fontWeight:"bold",borderRadius:"2px",background:tc[d.type]?.bg,color:tc[d.type]?.c}}>{d.type.toUpperCase()}</span><br/>{d.text}</div>))}
        </div>
        <div style={{display:"flex",gap:"6px",marginTop:"8px",justifyContent:"flex-end"}}>
          <VBtn onClick={()=>setShowSend(false)} small>Cancelar</VBtn>
          <VBtn onClick={sendToSheet} small disabled={sending} style={{background:"#d0d0ff",fontWeight:"bold"}}>{sending?"⏳ Enviando...":"📧 Enviar a Sheet"}</VBtn>
        </div>
      </div>)}
    </div>
  );
...
function HCEPortal({project,onSelectForm,onBack}){
  ...
  const evols=project.formularios.map((f,i)=>({
    fecha:new Date(Date.now()-i*3600000).toLocaleString("es-AR",{day:"2-digit",month:"2-digit",year:"2-digit",hour:"2-digit",minute:"2-digit"}),
    servicio:f.especialidad,
    nombre:f.estudio||`Formulario ${i+1}`,
    idx:i,
  }));
  return(
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr auto",gap:"8px",marginBottom:"8px",border:`1px solid ${S.borderDark}`,background:"#f0fff0"}}>
            <div style={{padding:"6px 10px"}}>
              <div style={{display:"flex",gap:"20px",alignItems:"baseline"}}>
                <div><span style={{color:S.teal,fontSize:"9px"}}>Paciente</span><div style={{fontSize:"20px",fontWeight:"bold"}}>{p.nombre||"[Nombre del Paciente]"}</div></div>
              </div>
              <div style={{display:"flex",gap:"12px",marginTop:"4px",fontSize:"10px"}}>
                <span><span style={{color:S.teal}}>Nº HC:</span> <b>{p.hc||"[HC]"}</b></span>
                <span><span style={{color:S.teal}}>Edad:</span> <b>{p.edad||"[Edad]"}</b></span>
                <span><span style={{color:S.teal}}>Fecha Ncto:</span> <b>{p.fechaNacimiento||"[Fecha]"}</b></span>
                <span><span style={{color:S.teal}}>Sexo:</span> <b>{p.sexo||"[Sexo]"}</b></span>
              </div>
            </div>
            <div style={{padding:"6px 10px",borderLeft:`1px solid ${S.borderDark}`,fontSize:"10px"}}>
              ...
              <div><b>Episodio:</b> {project.internacion.episodio||"[Episodio]"}</div>
              <div><b>Ingreso:</b> {project.internacion.ingreso||"[Fecha]"}</div>
              <div><b>Ubicación:</b> {project.internacion.ubicacion||"[Ubicación]"}</div>
              <div><b>Cant Días:</b> {project.internacion.cantDias||"[Días]"}</div>
            </div>
          </div>

          {/* Evoluciones list */}
          <div style={{border:`2px groove ${S.bg}`,padding:"4px"}}>
            <div style={{fontWeight:"bold",padding:"2px 4px",borderBottom:`1px solid ${S.borderDark}`,marginBottom:"4px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <span>📋 Evoluciones / Registros</span>
              <span style={{fontSize:"9px",color:"#666"}}>Hacé clic en un registro para revisarlo</span>
            </div>
            <div style={{border:`2px inset ${S.borderDark}`,background:"#fff"}}>
              <div style={{display:"grid",gridTemplateColumns:"110px 140px 1fr 30px",background:"#e8e8e8",fontWeight:"bold",fontSize:"9px",borderBottom:`1px solid ${S.borderDark}`}}>
                {["Fecha","Servicio","Evolución",""].map(h=>(<div key={h} style={{padding:"3px 4px",borderRight:"1px solid #ccc"}}>{h}</div>))}
              </div>
              {evols.map((ev,i)=>(
                <div key={i} onClick={()=>onSelectForm(ev.idx)}
                  style={{display:"grid",gridTemplateColumns:"110px 140px 1fr 30px",borderBottom:"1px solid #e0e0e0",cursor:"pointer",fontSize:"10px",background:i%2?"#f8f8f8":"#fff"}}
                  onMouseEnter={e=>e.currentTarget.style.background="#ffffcc"}
                  onMouseLeave={e=>e.currentTarget.style.background=i%2?"#f8f8f8":"#fff"}>
                  <div style={{padding:"3px 4px",borderRight:"1px solid #e0e0e0"}}>{ev.fecha}</div>
                  <div style={{padding:"3px 4px",borderRight:"1px solid #e0e0e0"}}>{ev.servicio}</div>
                  <div style={{padding:"3px 4px",borderRight:"1px solid #e0e0e0",fontWeight:"bold",color:S.blue}}>
                    📄 {ev.nombre}
                  </div>
                  <div style={{padding:"3px 4px",textAlign:"center",fontSize:"12px"}}>→</div>
                </div>
              ))}
              {!evols.length&&<div style={{padding:"20px",textAlign:"center",color:"#808080"}}>Sin registros</div>}
            </div>
          </div>
        </div>
      </div>
...
export default function App(){
  const[view,setView]=useState("editor"); // editor | hce | registro
  ...
  const updForm=(idx,newForm)=>setProject(p=>{const f=[...p.formularios];f[idx]=newForm;return{...p,formularios:f};});
  ...
  const exportProject=()=>{
    ...
    const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=`${project.paciente.nombre||"proyecto"}.json`;a.click();
  };
  const importProject=()=>{
    const input=document.createElement("input");input.type="file";input.accept=".json";
    input.onchange=e=>{const f=e.target.files[0];if(!f)return;const r=new FileReader();
      r.onload=ev=>{try{const d=JSON.parse(ev.target.result);if(d.formularios){setProject({...defaultProject(),...d});setView("editor");alert("✅ Importado");}else alert("❌ Formato inválido");}catch{alert("❌ Error");}};
      r.readAsText(f);};input.click();
  };

  if(view==="hce")return(
    <div style={{fontFamily:S.font,background:S.bg,minHeight:"100vh",padding:"6px"}}>
      <HCEPortal project={project} onBack={()=>setView("editor")} onSelectForm={idx=>{setSelectedFormIdx(idx);setView("registro");}}/>
    </div>
  );

  if(view==="registro")return(
    <div style={{fontFamily:S.font,background:S.bg,minHeight:"100vh",padding:"6px"}}>
      <RegistroView form={project.formularios[selectedFormIdx]} sheetUrl={project.sheetUrl} paciente={project.paciente} onBack={()=>setView("hce")}/>
    </div>
  );
  ...
  return(
    <div style={{fontFamily:S.font,fontSize:"11px",background:S.bg,minHeight:"100vh",padding:"6px"}}>
      ...
      <div style={{display:"flex",gap:"4px",marginBottom:"6px",alignItems:"center",padding:"4px",border:`2px groove ${S.bg}`,flexWrap:"wrap"}}>
        ...
        <VBtn active={view==="editor"} onClick={()=>setView("editor")}>🔧 Editor</VBtn>
        <VBtn onClick={()=>setView("hce")} style={{background:"#d0ffd0",fontWeight:"bold"}}>👁️ Ver como Usuario</VBtn>
        <Sep/>
        <VBtn onClick={exportProject} small>📤 Exportar</VBtn>
        <VBtn onClick={importProject} small>📥 Importar</VBtn>
        <div style={{flex:1}}/>
        <VBtn onClick={()=>{if(window.confirm("¿Nuevo proyecto?"))setProject(defaultProject());}} small style={{color:"#800000"}}>🔄 Nuevo</VBtn>
      </div>

      {/* PATIENT & PROJECT CONFIG */}
      <div style={{border:`2px outset ${S.borderLight}`,background:S.bg,marginBottom:"6px"}}>
        <div style={{background:S.blueGrad,color:S.white,fontWeight:"bold",padding:"3px 6px",fontSize:"11px"}}>📋 Datos del Paciente y Proyecto</div>
        <div style={{padding:"8px"}}>
      ...
      {/* FORMULARIOS TABS */}
      <div style={{border:`2px outset ${S.borderLight}`,background:S.bg}}>
        <div style={{background:S.blueGrad,color:S.white,fontWeight:"bold",padding:"3px 6px",fontSize:"11px",display:"flex",justifyContent:"space-between"}}>
          <span>📝 Formularios / Registros ({project.formularios.length})</span>
          <VBtn onClick={addForm} small style={{fontSize:"9px"}}>➕ Agregar Formulario</VBtn>
        </div>
        <div style={{padding:"4px 8px"}}>
          {/* Tabs */}
          <div style={{display:"flex",gap:"2px",marginBottom:"6px",flexWrap:"wrap"}}>
            {project.formularios.map((f,i)=>(
              <div key={f.id} style={{display:"flex",alignItems:"center",gap:"0"}}>
                <VBtn active={activeFormIdx===i} onClick={()=>setActiveFormIdx(i)} small>
                  {f.estudio||`Form ${i+1}`}
                </VBtn>
                {project.formularios.length>1&&(
                  <span onClick={()=>{if(window.confirm("¿Eliminar?"))removeForm(i);}} style={{cursor:"pointer",fontSize:"9px",color:"#800000",padding:"0 3px"}}>✕</span>
                )}
              </div>
            ))}
          </div>
          {/* Active form editor */}
          {project.formularios[activeFormIdx]&&(
            <FormEditor
              form={project.formularios[activeFormIdx]}
              setForm={nf=>updForm(activeFormIdx,nf)}
            />
          )}
        </div>
      </div>
